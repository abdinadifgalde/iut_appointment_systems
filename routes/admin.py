...
def update_status(appointment_id, status):
    ...
    if status == 'Rejected':
        ...
        if form.validate_on_submit():
            ...
            db.session.add(AppointmentTimeline(appointment_id=apt.id, status='Rejected', note=form.rejection_note.data))
...
def analytics_dashboard():
    ...
    # Busiest officers
    busiest = (db.session.query(Officer.name, func.count(Appointment.id).label('cnt'))
               .join(Appointment, Appointment.officer_id == Officer.id)
               .group_by(Officer.id)
               .order_by(func.count(Appointment.id).desc())
               .limit(5).all())

    cancellation_rate = round(rejected / total * 100, 1) if total else 0

    summary = {
        'total_appointments': total,
        'completed_appointments': completed,
        'cancelled_appointments': rejected,
        'pending_appointments': pending,
        'total_students': total_students,
        'total_officers': total_officers,
        'cancellation_rate': cancellation_rate,
        'busiest_officers': busiest,
    }

    return render_template('admin/analytics_dashboard.html', summary=summary)
